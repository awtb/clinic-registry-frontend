...
const buildRawRequester = (fetchFn: typeof fetch) => {
    return async (args: RequestArgs): Promise<ApiResult<unknown>> => {
        ...
        try {
            ...
            return { ok: false, status: response.status, error: buildParseError(e), headers: response.headers }
        }

        if (!response.ok) {
            return { ok: false, status: response.status, error: buildHttpError(response.status, payload), headers: response.headers }
        }
...
const parseWithSchema = <T>(schema: z.ZodType<T>, payload: unknown, status: number, headers: Headers): ApiResult<T> => {
